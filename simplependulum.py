...
ax1 = fig.add_subplot(212, aspect=2.0, autoscale_on=False,\
                xlim=(0.0,200*dt), ylim=(-0.1, pendulum.energy()+0.5))                
...
#xpos_text = ax.text(0.5, 0.5, '', transform=ax.transAxes)
...
def init():
    ...
    lineK.set_data([],[])
    lineE.set_data([],[])
    return line, time_text, energy_text, lineU, lineK, lineE
...
def animate(i):
    ...
    energy_text.set_text('energy, U, K = %.3f J, %.3f J, %.3f J' % (E, U, K))
